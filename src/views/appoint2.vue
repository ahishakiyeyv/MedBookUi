...
<div v-else>
  <Menu></Menu>
    ...
    <section>
        <div class="tabs">
                    <router-link to="/appointment" class="tab-item" >Tous (<span>{{this.$store.state.all_count}}</span>)</router-link>
            <router-link to="/tab2" class="tab-item selected">En Attente (<span>{{this.$store.state.attente_count}}</span>)</router-link>
...
export default {
    ...
    methods:{
        ...
        getAppointment(){
            axios
            .get('http://127.0.0.1:8000/api/status2')
            .then((res)=>{
